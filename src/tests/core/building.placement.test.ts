...
describe("placeBuilding", () => {
  ...
  it("connects a newly placed building when it is adjacent to an existing road", () => {
    const buildTileId = "tile_0_0";
    const roadTileId = "tile_1_0";
    const state: EconomySimulationState = {
      tick: 0,
      ageOfTeeth: 0,
      players: {
      ...
      territory: {
        ...
        tileIndex: { "0,0": buildTileId, "1,0": roadTileId },
      } as any,
      workers: {},
      transport: {
        jobs: {},
    ...
    const next = placeBuilding(state, "p1", "organHarvester", buildTileId);
    const placed = Object.values(next.buildings).find(b => b.type === "organHarvester")!;

    expect(placed.connectedToRoad).toBe(true);
  });
