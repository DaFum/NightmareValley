...
export default function TacticalMapPanel(): JSX.Element {
  ...
  const brief = useMemo(
    () => createTacticalMapBrief(useGameStore.getState().gameState, player1Id),
    [mapBucket]
  );
  ...
  return (
    <div className="tactical-map" aria-label="Tactical map">
      <button
        type="button"
        className={`macabre-panel tactical-map__toggle ${open ? 'active' : ''}`}
        aria-expanded={open}
        aria-controls="tactical-map-panel"
        onClick={() => setOpen((value) => !value)}
      >
        Map
      </button>
      {open ? (
        <section id="tactical-map-panel" className="tactical-map__panel macabre-panel" aria-label="Tactical map overview">
          ...
          <div className="tactical-map__next">
            <span>Next Chain</span>
            <strong>{brief.nextLabel}</strong>
            <small>{brief.markerCopy}</small>
          </div>
        </section>
      ) : null}
    </div>
